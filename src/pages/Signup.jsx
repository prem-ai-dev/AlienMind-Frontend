import { useState } from "react";
import { useNavigate } from "react-router-dom";
import api from "../lib/api";

export default function Signup() {
  const navigate = useNavigate();

  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    password: "",
  });

  const [showPassword, setShowPassword] = useState(false);
  const [success, setSuccess] = useState(null);
  const [error, setError] = useState(null);

  const handleChange = (e) => {
    setFormData((prev) => ({
      ...prev,
      [e.target.name]: e.target.value,
    }));
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(null);
    try {
        const response = await api.post("/signup/new_user", {
        name: formData.fullName,
        email: formData.email,
        password: formData.password,
        });
        console.log(response.data)
        localStorage.setItem("temp_token", response.data.access_token);
        setSuccess(response.data.user);
        setTimeout(() => navigate("/workspace-setup"), 4000);
        } catch (err) {
        if (err.response?.status === 409) {
            setError("Email is already in use.");
        } else {
            setError("Something went wrong. Please try again.");
        }
    }
  };

  return (
    <div className="relative flex min-h-screen flex-col overflow-hidden bg-[#0D0E12] text-[#E8E8EA]">
      {/* Background Glow */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-1/2 top-1/2 h-150 w-150 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#7F77DD]/15 blur-[180px]" />

        {[...Array(14)].map((_, i) => (
          <div
            key={i}
            className="absolute h-1 w-1 rounded-full bg-[#7F77DD]/50"
            style={{
              left: `${8 + i * 6}%`,
              top: `${50 + (i % 6) * 90}px`,
            }}
          />
        ))}
      </div>

      {/* Header */}
      <header className="relative z-10 px-8 py-7">
        <button
          onClick={() => navigate("/")}
          className="flex items-center gap-4"
        >
          <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#7F77DD] font-bold">
            A
          </div>

          <h1 className="text-3xl font-bold tracking-wide">
            ALIEN MIND
          </h1>
        </button>
      </header>

      {/* Signup Card */}
      <main className="relative z-10 flex flex-1 items-center justify-center px-6 py-10">
        <div className="w-full max-w-md rounded-3xl border border-[#7F77DD]/30 bg-[#101117] p-10 shadow-[0_0_60px_rgba(127,119,221,0.25)]">
          <h2 className="text-4xl font-bold">Create your workspace</h2>

          <p className="mt-3 text-[#6B6B76]">
            Start managing projects with AI
          </p>

          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            {/* Full Name */}
            <div>
              <label className="mb-2 block text-sm text-[#AFA9EC]">
                Full Name
              </label>

              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B6B76]">
                  👤
                </span>

                <input
                  type="text"
                  name="fullName"
                  value={formData.fullName}
                  onChange={handleChange}
                  placeholder="Enter your full name"
                  className="w-full rounded-xl border border-[#262830] bg-[#0D0E12] py-3 pl-12 pr-4 outline-none transition focus:border-[#7F77DD]"
                />
              </div>
            </div>

            {/* Email */}
            <div>
              <label className="mb-2 block text-sm text-[#AFA9EC]">
                Email
              </label>

              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B6B76]">
                  ✉
                </span>

                <input
                  type="email"
                  name="email"
                  value={formData.email}
                  onChange={handleChange}
                  placeholder="Enter your email"
                  className="w-full rounded-xl border border-[#262830] bg-[#0D0E12] py-3 pl-12 pr-4 outline-none transition focus:border-[#7F77DD]"
                />
              </div>
            </div>

            {/* Password */}
            <div>
              <label className="mb-2 block text-sm text-[#AFA9EC]">
                Password
              </label>

              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#6B6B76]">
                  🔒
                </span>

                <input
                  type={showPassword ? "text" : "password"}
                  name="password"
                  value={formData.password}
                  onChange={handleChange}
                  placeholder="Create a password"
                  className="w-full rounded-xl border border-[#262830] bg-[#0D0E12] py-3 pl-12 pr-14 outline-none transition focus:border-[#7F77DD]"
                />

                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-[#6B6B76] hover:text-white"
                >
                  {showPassword ? "🙈" : "👁"}
                </button>
              </div>
            </div>

            {/* Submit */}
            <button
              type="submit"
              className="mt-3 w-full rounded-xl bg-[#7F77DD] py-3 text-lg font-semibold transition hover:bg-[#9189ee]"
            >
              Create Account
            </button>
            {error && (
                <p className="text-sm text-red-400 text-center">{error}</p>
            )}
        </form>

          {/* Bottom Link */}
          <div className="mt-8 text-center text-[#6B6B76]">
            Already have an account?{" "}
            <button
              onClick={() => navigate("/login")}
              className="font-medium text-[#AFA9EC] transition hover:text-white"
            >
              Sign In
            </button>
          </div>
          {success && (
            <div className="mt-6 rounded-xl border border-[#262830] bg-[#0D0E12] p-4 text-sm">
                <div className="mb-2 flex justify-between text-[#6B6B76]">
                <span>Name</span><span className="text-[#E8E8EA]">{success.name}</span>
                </div>
                <div className="mb-2 flex justify-between text-[#6B6B76]">
                <span>Email</span><span className="text-[#E8E8EA]">{success.email}</span>
                </div>
                <div className="mb-2 flex justify-between text-[#6B6B76]">
                <span>User ID</span><span className="text-[#E8E8EA] text-xs">{success.user_id}</span>
                </div>
                <div className="mb-2 flex justify-between text-[#6B6B76]">
                <span>Joined</span>
                <span className="text-[#E8E8EA] text-xs">
                {new Date(success.created_at).toLocaleDateString()}
                </span>
                </div>
                <p className="mt-3 text-center text-green-400 font-medium">
                ✓ User created successfully
                </p>
            </div>
            )}
        </div>
      </main>

      {/* Footer */}
      <footer className="relative z-10 flex flex-col items-center justify-between gap-6 border-t border-[#262830] px-8 py-8 text-[#6B6B76] md:flex-row">
        <span>AlienMind</span>

        <div className="flex gap-8">
          <a href="#" className="hover:text-[#AFA9EC]">
            Privacy
          </a>

          <a href="#" className="hover:text-[#AFA9EC]">
            Terms
          </a>

          <a href="#" className="hover:text-[#AFA9EC]">
            Contact
          </a>

          <a href="#" className="hover:text-[#AFA9EC]">
            GitHub
          </a>
        </div>
      </footer>
    </div>
  );
}